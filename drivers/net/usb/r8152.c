...
/* Version Information */
#define DRIVER_VERSION "v2.17.1.dsgonly (2023/06/13)"
...
static unsigned int agg_buf_sz = 32768;
				     ...
				     bool in_resume)
{
	...
	r8152_eth_hw_addr_set(netdev, addr->sa_data);
...
static int set_ethernet_addr(struct r8152 *tp, bool in_resume)
{
	...
	if (tp->version == RTL_VER_01)
		r8152_eth_hw_addr_set(dev, sa.sa_data);
	else
			 ...
			 const struct usb_device_id *id)
{
	...
	//netdev->features &= ~(NETIF_F_TSO | NETIF_F_TSO6 | NETIF_F_FRAGLIST);
	//netdev->hw_features &= ~(NETIF_F_TSO | NETIF_F_TSO6 | NETIF_F_FRAGLIST);
	tp->sg_use = false;
